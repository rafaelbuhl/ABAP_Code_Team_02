CLASS zcl_cs02_statistics_03 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_cs02_statistics_03 IMPLEMENTATION.
  ...
  METHOD zif_statistics02~averagesales.
   ...
   DATA(lv_current_date) = cl_abap_context_info=>get_system_date( ).
    ...
    TRY.
      " Auswahl der Bestellungen mit Anpassung des order_date
        ...
        FIELDS customerid,
               CAST( AVG( currency_conversion( amount = order_total,
                                               ...
                                               exchange_rate_date = CASE
                                                 WHEN substring( order_date, 1, 4 ) = @lv_fiscal_year
                                                 THEN order_date
                                                 ELSE @lv_current_date
                                               END ) ) AS DEC( 12,2 ) ) AS Average,
               'EUR' AS Currency,
               COUNT( orderid ) AS NumberOfOrders
        WHERE customerid = @i_customer
          AND order_total <> 0
          AND status <> 'ST'
        GROUP BY customerid
    ...
    CATCH cx_sy_open_sql_db INTO DATA(lx_sql_error).
      " Fehlerbehandlung, z.B. Protokollierung oder Meldung
  ...
  METHOD zif_statistics02~daysales.
   ...
   DATA(lv_current_date) = cl_abap_context_info=>get_system_date( ).
    ...
    lv_start_of_year = lv_today(4) && '0102'.
